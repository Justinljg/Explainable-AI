...
WORKDIR /app
COPY src/sl.py /app/src/sl.py
COPY conf/base/logging.yaml /app/conf/base/logging.yaml
...
CMD ["python", "-m", "streamlit", "run", "src/sl.py", "--server.port=8501", "--server.address=0.0.0.0"]